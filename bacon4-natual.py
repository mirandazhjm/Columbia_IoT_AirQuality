...
def save_data():
	...
	file1 = open('bacon4.csv','w')
	file2 = open('bacon4.txt','w')
...
def data_proc():
	...
	# print model_ransac_voc.estimator_.coef_
	...
	# popt_co2, pcov = curve_fit(func, xx, yy_co2)
	# popt_voc, pcov = curve_fit(func, xx, yy_voc)
	# popt_dust, pcov = curve_fit(func, xx, yy_dust)
	...
	# plt.subplot(131)
	# f0 = plt.figure()
	plt.suptitle('Data on conditon: Frying bacon with natual decay')
	ax0 = plt.subplot(311) 
	# ax0.plot(co2_data,linewidth=2)
	ax0.set_title('CO2')
	# plt.plot(range(change_id - start_id),co2_data[:(change_id - start_id)],'b',label="test1",linewidth=2)
	# plt.plot(range(change_id - start_id-1, stop_id - start_id, 1),co2_data[(change_id - 1 - start_id):(stop_id - start_id)],'g',label="test1",linewidth=2)
	# plt.plot(range(change_id - start_id-1, stop_id+1- start_id,1),co2_data[(end_id - 1 - start_id):],'g',label="test1",linewidth=2)
	...
	ax0.plot(xx, func(xx, *popt_co2), 'r-', label="Fitted Curve")
	...
	# ax1.plot(range(change_id - start_id-1, stop_id - start_id, 1),voc_data[(change_id - 1 - start_id):(stop_id - start_id)],'g',linewidth=2)
	...
	ax1.plot(xx, func(xx, *popt_voc), 'r-', label="Fitted Curve")
	...
	# ax2.plot(range(change_id - start_id-1, stop_id - start_id, 1),dust_data[(change_id - 1 - start_id):(stop_id - start_id)],'g',linewidth=2)
	...
	# ax1.plot(xx, func(xx, *popt_dust), 'r-', label="Fitted Curve")
	...
	#plt.close(3)
	plt.figure(2)
	plt.suptitle('Data on condition: Frying bacon with natual decay')
	ax3 = plt.subplot(211)
	ax3.plot(range(change_id - start_id),tem_data[:(change_id - start_id)],'b',linewidth=2)
	ax3.plot(range(change_id - start_id-1, stop_id - start_id, 1),tem_data[(change_id - 1 - start_id):(stop_id - start_id)],'g',linewidth=2)
