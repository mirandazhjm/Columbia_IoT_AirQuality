...
refer_id = change_id
refer_date = "2016-05-07 20:34:10"
...
def save_data():
	...
	file1 = open('bacon3.csv','w')
	file2 = open('bacon3.txt','w')
...
def data_proc():
	...
	yy_co2 = np.array(co2_data[(change_id - start_id):(end_id - start_id+1)])
	yy_co2 = yy_co2.reshape((len(yy_co2),1))
	yy_voc = np.array(voc_data[(change_id - start_id):(end_id - start_id+1)])
	yy_voc = yy_voc.reshape((len(yy_voc),1))
	yy_dust = np.array(dust_mean[(change_id - start_id):(end_id - start_id+1)])
	yy_dust = yy_dust.reshape((len(yy_dust),1))

	yyy_co2 = np.array(co2_data[(end_id - start_id):(stop_id - start_id+1)])
	yyy_co2 = yyy_co2.reshape((len(yyy_co2),1))
	yyy_voc = np.array(voc_data[(end_id - start_id):(stop_id - start_id+1)])
	yyy_voc = yyy_voc.reshape((len(yyy_voc),1))
	yyy_dust = np.array(dust_mean[(end_id - start_id):(stop_id - start_id+1)])
	yyy_dust = yyy_dust.reshape((len(yyy_dust),1))

	line_x = np.arange(0,(change_id - start_id))
	line_xx = np.arange((change_id - start_id),(end_id - start_id+1))
	...
	model_ransac_co2_2 = linear_model.RANSACRegressor(linear_model.LinearRegression())
	model_ransac_co2_2.fit(xxx, yyy_co2)
	inlier_mask_co2_2 = model_ransac_co2_2.inlier_mask_
	outlier_mask_co2_2 = np.logical_not(inlier_mask_co2_2)
	line_y_ransac_co2_2 = model_ransac_co2_2.predict(line_xxx[:, np.newaxis])

	model_ransac_voc_2 = linear_model.RANSACRegressor(linear_model.LinearRegression())
	model_ransac_voc_2.fit(xxx, yyy_voc)
	inlier_mask_voc_2 = model_ransac_voc_2.inlier_mask_
	outlier_mask_voc_2 = np.logical_not(inlier_mask_voc_2)
	line_y_ransac_voc_2 = model_ransac_voc_2.predict(line_xxx[:, np.newaxis])

	model_ransac_dust_2 = linear_model.RANSACRegressor(linear_model.LinearRegression())
	model_ransac_dust_2.fit(xxx, yyy_dust)
	inlier_mask_dust_2 = model_ransac_dust_2.inlier_mask_
	outlier_mask_dust_2 = np.logical_not(inlier_mask_dust_2)
	line_y_ransac_dust_2 = model_ransac_dust_2.predict(line_xxx[:, np.newaxis])
	print model_ransac_dust_2.estimator_.coef_

	## plot the graph	
	# plt.subplot(131)
	# f0 = plt.figure()
	plt.suptitle('Data on conditon: Frying bacon with opening Airpurifier')
	ax0 = plt.subplot(311) 
	# plt.plot(range(change_id - start_id),co2_data[:(change_id - start_id)],'b',label="test1",linewidth=2)
	# plt.plot(range(change_id - start_id-1, end_id - start_id, 1),co2_data[(change_id - 1 - start_id):(end_id - start_id)],'r',label="test1",linewidth=2)
	# plt.plot(range(end_id - start_id-1, stop_id+1- start_id,1),co2_data[(end_id - 1 - start_id):],'g',label="test1",linewidth=2)
	...
	ax0.plot(line_xxx, line_y_ransac_co2_2, '-b', label='RANSAC regressor',linewidth=2)
	ax0.plot(xxx[inlier_mask_co2_2], yyy_co2[inlier_mask_co2_2], '.g', label='Inliers')
	ax0.plot(xxx[outlier_mask_co2_2], yyy_co2[outlier_mask_co2_2], '.r', label='Outliers')
	...
	# ax1.plot(range(change_id - start_id-1, end_id - start_id, 1),voc_data[(change_id - 1 - start_id):(end_id - start_id)],'r',linewidth=2)
	# ax1.plot(range(end_id - start_id-1, stop_id+1- start_id,1),voc_data[(end_id - 1 - start_id):],'g',linewidth=2)
	...
	ax1.plot(line_xxx, line_y_ransac_voc_2, '-b', label='RANSAC regressor',linewidth=2)
	ax1.plot(xxx[inlier_mask_voc_2], yyy_voc[inlier_mask_voc_2], '.g', label='Inliers')
	ax1.plot(xxx[outlier_mask_voc_2], yyy_voc[outlier_mask_voc_2], '.r', label='Outliers')
	...
	# plt.figure(3)
	# plt.subplot(133)
	# ax2.plot(dust_data,'b',dust_mean,'r',linewidth=2)
	# ax2.plot(dust_mean,'y',linewidth=2)
	# ax2.plot(range(change_id - start_id),dust_data[:(change_id - start_id)],'b',linewidth=2)
	# ax2.plot(range(change_id - start_id-1, end_id - start_id, 1),dust_data[(change_id - 1 - start_id):(end_id - start_id)],'r',linewidth=2)
	# ax2.plot(range(end_id - start_id-1, stop_id+1- start_id,1),dust_data[(end_id - 1 - start_id):],'g',linewidth=2)
	...
	ax2.plot(line_xxx, line_y_ransac_dust_2, '-b', label='RANSAC regressor',linewidth=2)
	ax2.plot(xxx[inlier_mask_dust_2], yyy_dust[inlier_mask_dust_2], '.g', label='Inliers')
	ax2.plot(xxx[outlier_mask_dust_2], yyy_dust[outlier_mask_dust_2], '.r', label='Outliers')
	
	ax2.set_title('Dust')
	# ax2.set_title('Dust data')
	...
	plt.figure(2)
	plt.suptitle('Data on condition: Frying bacon with opening the Airpurifier')
	ax3 = plt.subplot(211)
	# ax3.plot(tem_data,'b',linewidth=2)
	ax3.plot(range(change_id - start_id),tem_data[:(change_id - start_id)],'b',linewidth=2)
	ax3.plot(range(change_id - start_id-1, end_id - start_id, 1),tem_data[(change_id - 1 - start_id):(end_id - start_id)],'r',linewidth=2)
	ax3.plot(range(end_id - start_id-1, stop_id+1- start_id,1),tem_data[(end_id - 1 - start_id):],'g',linewidth=2)
	...
	ax4.plot(range(change_id - start_id-1, end_id - start_id, 1),hum_data[(change_id - 1 - start_id):(end_id - start_id)],'r',linewidth=2)
	ax4.plot(range(end_id - start_id-1, stop_id+1- start_id,1),hum_data[(end_id - 1 - start_id):],'g',linewidth=2)
